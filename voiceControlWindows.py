import requests
import sys
import pyaudio  # For audio recording on Windows
import numpy as np
...
def record_audio(duration, samplerate, channels):
    ...
    try:
        ...
        # Open stream with correct format
        ...
        # Write recorded data to a buffer
        wav_buffer = io.BytesIO()
        write(wav_buffer, samplerate, np.frombuffer(b''.join(frames), dtype=np.int16))
        wav_buffer.seek(0)
