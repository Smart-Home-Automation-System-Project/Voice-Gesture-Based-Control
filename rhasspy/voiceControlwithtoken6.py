...
def generate_control_token(intent_data):
    ...
    # Helper function to find device ID by name
    ...
    if intent_name == 'TurnOnDevice':
        ...
        if device_id:
            token = {"id": device_id, "state": "on"}
    elif intent_name == 'TurnOffDevice':
        device_name = slots.get('device', '')
        device_id = find_device_id(device_name)
        if device_id:
            token = {"id": device_id, "state": "off"}
    ...
    # Batch mode control
        ...
        if control_type in ['lights', 'switches', 'doors'] and state in ['on', 'off', 'lock', 'unlock']:
            # Collect IDs for the specified type
            target_type = 'light' if control_type == 'lights' else 'switch' if control_type == 'switches' else 'door'
            device_ids = [device_id for device_id, _, device_type in DEVICES if device_type == target_type]
            if device_ids:
                # For batch control, send a single token with a list of IDs
                token = {"id": device_ids, "state": state}
